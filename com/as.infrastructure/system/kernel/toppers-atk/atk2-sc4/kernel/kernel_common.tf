...
$  の責任を負わない．
$
$  $Id: kernel_common.tf 504 2015-12-24 01:22:56Z witz-itoyo $
$
		...
		$FOREACH tpid TSK.TP.ID_LIST$
			...
			$monitoring = 0$
				...
				$IF EQ(TSK.TP.TIMEFRAME[tpid], NULL)$
					$TAB$$TAB${ 0x00U, 0x00U },$NL$
				$ELSE$
					$timeframe = TSK.TP.TIMEFRAME_CLOCK[tpid]$
					$IF (+AT(timeframe, 0) == 0) && (+AT(timeframe, 1) == 0)$
						$ERROR$
							$FORMAT("task time frame is 0 tick in %s sec.", TSK.TP.TIMEFRAME[tpid])$
						$END$
					$END$
					$TAB$$TAB${ $FORMAT("0x%0xU", +AT(timeframe, 0))$, $FORMAT("0x%0xU", +AT(timeframe, 1))$ },$NL$
				$END$
				$IF EQ(TSK.TP.EXECBUDGET[tpid], NULL)$
					$TAB$$TAB$0x00U,$NL$
				$ELSE$
					$IF +TSK.TP.EXECBUDGET_CLOCK[tpid] == 0$
						$ERROR$
							$FORMAT("task exccution budget is 0 tick in %s sec.", TSK.TP.EXECBUDGET[tpid])$
						$END$
					$END$
					$TAB$$TAB$$FORMAT("0x%0xU", +TSK.TP.EXECBUDGET_CLOCK[tpid])$,$NL$
				$END$
				$IF EQ(TSK.TP.OSINTLOCK[tpid], "TRUE")$
					$monitoring = monitoring | 0x01$
				$END$
				$IF EQ(TSK.TP.RESLOCK[tpid], "TRUE")$
					$monitoring = monitoring | 0x02$
				$END$
				$TAB$$TAB$$FORMAT("0x0%xU", monitoring)$$NL$
				$found = 1$
			$END$
		$END$

...
$  の責任を負わない．
$
$  $Id: kernel.tf 504 2015-12-24 01:22:56Z witz-itoyo $
$
...
$FOREACH tplist TSK.TP.ID_LIST$
	$IF LENGTH(TSK.TP.OSINTLOCK[tplist])$
		$TSK.TP.OSINTLOCK[tplist] = IS_TRUE(TSK.TP.OSINTLOCK[tplist], TSK.TP.TEXT_LINE[tplist])$
	$ELSE$
		$TSK.TP.OSINTLOCK[tplist] = VALUE("FALSE", 0)$
	$END$
	$IF LENGTH(TSK.TP.RESLOCK[tplist])$
		$TSK.TP.RESLOCK[tplist] = IS_TRUE(TSK.TP.RESLOCK[tplist], TSK.TP.TEXT_LINE[tplist])$
	$ELSE$
		$TSK.TP.RESLOCK[tplist] = VALUE("FALSE", 0)$
	$END$
$END$
...
$
$ PreTaskHookまたはPostTaskHookが設定されている場合はWarningを出力する [OS562]
$
$IF HOOK.PRETASKHOOK[1] || HOOK.POSTTASKHOOK[1]$
	$WARNING$Timing protection is configured together with Pre- or PostTaskHook$END$
$END$
...
$FOREACH tfnid OSAP.TFN.ID_LIST$
	...
	$TFN.TFBUDGET[tfnid] = OSAP.TFN.TFBUDGET[tfnid]$
...
$		// 割込み要求ラインの有効/無効を制御出来ないにも関わらず，OsIsrInterruptSourceがDISABLEになっている場合はエラーとする  
	...
	$IF EQ(ISR.CATEGORY[isrid], "CATEGORY_1")$
		...
		$ELSE$
			$ISR.STKSZ[isrid] = 0$
		$END$
		$FOREACH isrtpid ISR.TP.ID_LIST$
			$IF ISR.TP.PARENT[isrtpid] == isrid$
				$ERROR ISR.TEXT_LINE[isrid]$$FORMAT(_("OsIsrTimingProtection of C1ISR OsIsr(%1%) shall not defined"), isrid)$$END$
			$END$
		$END$
...
$IF LENGTH(ALM.ACTION.CALLBACK.ID_LIST)$
	$ERROR ALM.ACTION.CALLBACK.TEXT_LINE[1]$$FORMAT(_("OsAlarmCallback can't be used in SC4"))$$END$
$END$
...
$IF LENGTH(OS.SC[1]) && !EQ(OS.SC[1], "SC4")$
	$ERROR OS.TEXT_LINE[1]$$FORMAT(_("OsScalabilityClass must be SC4"))$$END$
$END$
...
$FOREACH isrid ISR.ID_LIST$
$	// INTNOがタイミング保護監視タイマと重複している場合
	$IF INTNO_TP_FAULT == ISR.INTNO[isrid]$
		$ERROR ISR.TEXT_LINE[isrid]$$FORMAT(_("OsIsrInterruptNumber(%1%) of OsIsr(%2%) and INTNO_TP_FAULT are duplicated"), ISR.INTNO[isrid], isrid)$$END$
	$END$
	$IF INTNO_TP_TIMER == ISR.INTNO[isrid]$
		$ERROR ISR.TEXT_LINE[isrid]$$FORMAT(_("OsIsrInterruptNumber(%1%) of OsIsr(%2%) and INTNO_TP_TIMER are duplicated"), ISR.INTNO[isrid], isrid)$$END$
	$END$
...
$		// C1ISRの割込み優先度がタイミング保護用割込み優先度(MIN_PRI_ISR2 - 1)以上である場合
		$IF -ISR.INTPRI[isrid] >= (MIN_PRI_ISR2 - 1)$
			$ERROR ISR.TEXT_LINE[isrid]$$FORMAT(_("OsIsrInterruptPriority(%1%) of OsIsr(%2%) is lower than or equal to C2ISR min priority(%3%)"), ISR.INTPRI[isrid], isrid, -(MIN_PRI_ISR2-1))$$END$
		$END$
...
$
$ C1ISRが存在する場合はWarningを出力する [OS045]
$
$IF LENGTH(ISR1.ID_LIST) > 0$
	$WARNING$Timing protection is configured together with Category 1 interrupts$END$
$END$
...
$	// 内部リソースにリソース占有時間バジェットが設定されていたらエラーとする．
	$IF EQ(RES.RESATR[resid], "INTERNAL")$
		$FOREACH restpid RES.RESTP.ID_LIST$
			$IF RES.RESTP.PARENT[restpid] == resid$
				$IF LENGTH(RES.RESTP.RESTPLOCKBUDGET[restpid])$
					$ERROR RES.TEXT_LINE[resid]$$FORMAT(_("internal resource OsResource(%1%) can't be assigned a OsResourceLockBudget"), restpid)$$END$
				$END$
			$END$
		$END$
	$END$
...
$		//リンクリソース元の最大バジェット値取得
		$FOREACH restpid RES.RESTP.ID_LIST$
			$IF RES.RESTP.PARENT[restpid] == lnkresid$
				$restplockbudget = RES.RESTP.RESTPLOCKBUDGET[restpid]$
			$END$
		$END$


$		//同一リンクリソースの最大バジェット値計算
		$FOREACH rinkid RES.ID_LIST$
			$IF EQ(RES.RESATR[rinkid], "LINKED")$
				$IF RES.LINKEDRESID[rinkid] == lnkresid$
					$FOREACH restpid RES.RESTP.ID_LIST$
						$IF RES.RESTP.PARENT[restpid] == rinkid$
							$IF LENGTH(RES.RESTP.RESTPLOCKBUDGET[restpid])$
								$IF restplockbudget < RES.RESTP.RESTPLOCKBUDGET[restpid]$
									$restplockbudget = RES.RESTP.RESTPLOCKBUDGET[restpid]$
								$END$
							$END$
						$END$
...
$		// リンクリソース中最大バジェット値を代入
		$FOREACH restpid RES.RESTP.ID_LIST$
			$IF RES.RESTP.PARENT[restpid] == resid$
				$RES.RESTP.RESTPLOCKBUDGET[restpid] = restplockbudget$
				$flg = 1$
			$END$
			$IF RES.RESTP.PARENT[restpid] == lnkresid$
				$RES.RESTP.RESTPLOCKBUDGET[restpid] = restplockbudget$
			$END$
		$END$
$		// リンクリソースでタイミング保護コンテナが設定されていない場合の処理
		$IF flg == 0$
			$IF restplockbudget != 0$
				$nexttpid = +restpid + 1$
				$RES.RESTP.ID_LIST = APPEND(RES.RESTP.ID_LIST, nexttpid)$
				$RES.RESTP.PARENT[nexttpid] = resid$
				$RES.RESTP.RESTPLOCKBUDGET[nexttpid] = restplockbudget$
			$END$
		$END$
	$END$
$END$
...
$ // OS割込み禁止時間バジェットが設定されているかチェック
$osfound = 0$
$IF LENGTH(OSTP.ID_LIST)$
	$IF LENGTH(OSTP.OSINTLOCKBUDGET[1])$
		$osfound = 1$
	$END$
$END$
...
$	// 非信頼タスクに実行時間バジェットが設定されていない場合，warningを出力
	$IF !OSAP.TRUSTED[TSK.OSAPID[tskid]]$
$		// 非信頼タスクの場合
		$found = 0$
		$FOREACH tpid TSK.TP.ID_LIST$
			$IF TSK.TP.PARENT[tpid] == tskid$
				$found = 1$
$				// 非信頼タスクにタイミング保護コンテナが設定されているが，実行時間バジェットが設定されていない場合	
				$IF EQ(TSK.TP.EXECBUDGET[tpid], NULL)$
					$WARNING TSK.TEXT_LINE[tskid]$$FORMAT(_("Non Trusted OsTask(%1%) need to set the timing protection"), tskid)$$END$
				$END$
			$END$
		$END$
$		// 非信頼タスクにタイミング保護コンテナが設定されていない場合
		$IF found == 0$
			$IF !EQ(tskid, "OS_GEN_RESTARTTASK")$
				$WARNING TSK.TEXT_LINE[tskid]$$FORMAT(_("Non Trusted OsTask(%1%) need to set the timing protection"), tskid)$$END$
			$END$
		$END$
	$END$
...
$	// タイミング保護パラメータの生成
	$FOREACH tpid TSK.TP.ID_LIST$
		$IF TSK.TP.PARENT[tpid] == tskid$
			$IF !EQ(TSK.TP.TIMEFRAME[tpid], NULL)$
				$TSK.TP.TIMEFRAME_CLOCK[tpid] = SEC_TO_TIMEFRAME(TSK.TP.TIMEFRAME[tpid])$
			$END$
			$IF !EQ(TSK.TP.EXECBUDGET[tpid], NULL)$
				$TSK.TP.EXECBUDGET_CLOCK[tpid] = SEC_TO_CLOCK(TSK.TP.EXECBUDGET[tpid])$
			$ELSE$
				$TSK.TP.EXECBUDGET_CLOCK[tpid] = 0$
			$END$
		$END$
	$END$
$	//OS割込み禁止バジェットが設定されていなければ，全タスクのOS割込み禁止監視フラグをオフにする
	$IF osfound == 0$
		$FOREACH tplist TSK.TP.ID_LIST$
			$IF TSK.TP.PARENT[tplist] == tskid$
				$TSK.TP.OSINTLOCK[tplist] = "FALSE"$
			$END$
		$END$
	$END$
$END$
...
$	// OSアプリケーションは信頼OSアプリケーションか?
	...
	$END$
$
$ 信頼関数バジェットが設定されていない場合はWarningを出力する
$
	$IF !LENGTH(TFN.TFBUDGET[tfnid])$
		$WARNING TFN.TEXT_LINE[tfnid]$$FORMAT(_("OsApplicationTrustedFunction(%1%) need to set the timing protection"), tfnid)$$END$
	$END$
$END$
...
#define CFG_USE_SCALABILITYCLASS4$NL$
$NL$
...
$ カウンタオブジェクトのOS定数の出力
$FOREACH cntid CNT.ID_LIST$
	#define OSMAXALLOWEDVALUE_$cntid$$TAB$((TickType) $+CNT.MAXALLOWED[cntid]$U)$NL$
	#define OSTICKSPERBASE_$cntid$$TAB$((TickType) $+CNT.TICKSPERBASE[cntid]$U)$NL$
	#define OSMINCYCLE_$cntid$$TAB$((TickType) $+CNT.MINCYCLE[cntid]$U)$NL$
$END$
$NL$
...
const sint32					intpri_tp			= INTPRI_TP;$NL$
$NL$
...
$		// 未指定か，NULL(0)か0を指定した場合，タスクスタックサイズのデフォルト設定
		$IF LENGTH(TSK.STKSZ[tskid]) && TSK.STKSZ[tskid] != 0$
			$TSK.STKSZ[tskid] = VALUE(CONCAT(+TSK.STKSZ[tskid],"U"),+TSK.STKSZ[tskid])$
		$ELSE$
			$TSK.STKSZ[tskid] = VALUE(CONCAT(+DEFAULT_TASKSTKSZ,"U"),+DEFAULT_TASKSTKSZ)$
...
$IF LENGTH(OSTP.ID_LIST)$
	$OSTP.OSINTLOCKBUDGET_CLOCK[1] = SEC_TO_CLOCK(OSTP.OSINTLOCKBUDGET[1])$
	$IF OSTP.OSINTLOCKBUDGET_CLOCK[1] == 0$
		$ERROR$
			$FORMAT("C2ISR lock budget is 0 tick in %s sec.", OSTP.OSINTLOCKBUDGET[1])$
		$END$
	$END$
	const TickType osinterruptlock_budget = $FORMAT("0x%0xU", OSTP.OSINTLOCKBUDGET_CLOCK[1])$;$NL$
$ELSE$
	const TickType osinterruptlock_budget = 0x00U;$NL$
$END$
...
$           タスクのみが扱うリソースの場合
			$TAB$$TAB$$TMAX_TPRI - RES.CEILPRI[resid]$,$NL$
		$ELSE$
$           ISRも扱うリソースの場合
			$TAB$$TAB$$RES.CEILPRI[resid]$,$NL$
		$END$
		$TAB$$TAB$$FORMAT("0x%08xU", +RES.ACSBTMP[resid])$,$NL$

		$found = 0$
		$FOREACH restpid RES.RESTP.ID_LIST$
			$IF RES.RESTP.PARENT[restpid] == resid$
				$RES.RESTP.RESTPLOCKBUDGET_CLOCK[restpid] = SEC_TO_CLOCK(RES.RESTP.RESTPLOCKBUDGET[restpid])$
				$IF RES.RESTP.RESTPLOCKBUDGET_CLOCK[restpid] == 0$
					$ERROR$
						$FORMAT("resource lock budget is 0 tick in %s sec.", RES.RESTP.RESTPLOCKBUDGET[restpid])$
					$END$
				$END$
				$TAB$$TAB$$FORMAT("0x%0xU", RES.RESTP.RESTPLOCKBUDGET_CLOCK[restpid])$$NL$
				$found = 1$
			$END$
		$END$
		$IF found == 0$
...
$END$
$IF LENGTH(TSK.TP.ID_LIST) || LENGTH(ISR.TP.ID_LIST)$
$TAB$tp_initialize();$NL$
$END$
$IF LENGTH(CNT.ID_LIST)$
...
$IF LENGTH(ISR2.ID_LIST)$
	...
	$JOINEACH isrid ISR2.ID_LIST ",\n"$
		...
		&(osapcb_table[$ISR.OSAPID[isrid]$]), $FORMAT("0x%08xU, ", +ISR.ACSBTMP[isrid])$

$		// タイミング保護初期化ブロックの出力
		$found = 0$
		$FOREACH tpid ISR.TP.ID_LIST$
			$IF ISR.TP.PARENT[tpid] == isrid$
				$timeframe = SEC_TO_TIMEFRAME(ISR.TP.ISRTIMEFRAME[tpid])$
				$IF (AT(timeframe, 0) == 0) && (AT(timeframe, 1) == 0)$
					$ERROR$
						$FORMAT("C2ISR time frame is 0 tick in %s sec.", ISR.TP.ISRTIMEFRAME[tpid])$
					$END$
				$END$
				{ $FORMAT("0x%0xU", AT(timeframe, 0))$, $FORMAT("0x%0xU", AT(timeframe, 1))$ }$SPC$
				$found = 1$
			$END$
		$END$
		$IF found == 0$
...
$ 		// NULL(0)か0を指定した場合，デフォルト設定
		...
		$END$
		$TAB${$NL$
		$TAB$$TAB$&$TFN.FUNC[tfnid]$,$NL$
		$TAB$$TAB$$+TFN.STKSZ[tfnid]$U,$NL$

		$IF LENGTH(TFN.TFBUDGET[tfnid])$
			$TFBUDGET_CLOCK = SEC_TO_CLOCK(TFN.TFBUDGET[tfnid])$
			$TAB$$TAB$$FORMAT("0x%0xU", AT(TFBUDGET_CLOCK, 0))$$NL$
		$ELSE$
			$TAB$$TAB$0x00U$NL$
		$END$
		$TAB$}$NL$
...
/***** Timing Protection *****/$NL$$NL$

$INTPRI_TP = MIN_PRI_ISR2 - 1$

#define INTPRI_TP$TAB$($INTPRI_TP$)$NL$
...
$ タイミング保護関連データの受け渡し
$FOREACH tplist TSK.TP.ID_LIST$
	$$TSK.TP.EXECBUDGET_CLOCK[$+tplist$] = $TSK.TP.EXECBUDGET_CLOCK[tplist]$$$$NL$
	$$TSK.TP.TIMEFRAME_CLOCK[$+tplist$] = {$TSK.TP.TIMEFRAME_CLOCK[tplist]$}$$$NL$
	$$TSK.TP.OSINTLOCK[$+tplist$] = "$TSK.TP.OSINTLOCK[tplist]$"$$$NL$
	$$TSK.TP.RESLOCK[$+tplist$] = "$TSK.TP.RESLOCK[tplist]$"$$$NL$
	$NL$
$END$

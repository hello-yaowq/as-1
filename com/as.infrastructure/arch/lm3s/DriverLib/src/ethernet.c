...
//! to SysCtlClockGet()).
//!
//! This function replaces the original EthernetInit() API and performs the same
//! actions.  A macro is provided in <tt>ethernet.h</tt> to map the original
//! API to this API.
//!
...
//! time stamp control register provides settings that enable support logic in
//! the controller that allow the use of the General Purpose Timer 3 to capture
//! time stamps for the transmitted and received packets.
//!
...
//! can be read using the \e EthernetPHYRead() API function.
...
//! during reception of a frame.  This error can indicated a length mismatch,
//! a CRC failure, or an error indication from the PHY.
...
//! failure during the backoff process, or an invalid length stored in the TX
//! FIFO.

...
(defun mktags (RootDir)
  ...
  ;;(if (eq system-type 'windows-nt)
  (shell-command (format "cd %s && rm -f TAGS cscope.files cscope.out && find ./ -name *.[chCH] |xargs -i etags -a {} && find ./ -name *.[chCH] -print > cscope.files && cscope -b -i cscope.files -f cscope.out" RootDir))
  ;;)
